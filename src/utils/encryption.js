...
/**
 * Get UTC timestamp (guaranteed timezone-independent)
 * Returns Unix timestamp in seconds
 */
const getUTCTimestamp = () => {
    // Date.now() is ALWAYS UTC (milliseconds since Unix Epoch)
    const utcMilliseconds = Date.now();
    const utcSeconds = Math.floor(utcMilliseconds / 1000);

    // Log for debugging
    const date = new Date(utcMilliseconds);
    console.log('🕐 UTC Timestamp Details:', {
        milliseconds: utcMilliseconds,
        seconds: utcSeconds,
        iso: date.toISOString(), // Always UTC
        utcString: date.toUTCString() // Always UTC
    });
...
/**
 * ✅ FIXED: Encrypt X-API-KEY header with guaranteed UTC timestamp
 */
export function encryptXAPIKEY() {
    ...
    // ✅ Use helper function to get UTC timestamp
    const timestamp = getUTCTimestamp();

    console.log('🔐 Encrypting API Key with UTC timestamp:', timestamp);

    const plainText = `${apiKey}///${timestamp}`;
    console.log('📝 Plain text format:', plainText.substring(0, 30) + '...');
    ...
    console.log('✅ API Key encrypted with UTC timestamp');
...
export const testEncryption = async () => {
    ...
    try {
        const testData = {
            test: "Hello, World!",
            number: 42,
            timestamp: getUTCTimestamp() // Use UTC timestamp
        };
...
/**
 * Export UTC timestamp helper for external use
 */
